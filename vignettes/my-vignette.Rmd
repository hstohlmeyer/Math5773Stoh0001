...
Here is an example of the `validitycheck` function using the camplake data found in `library(s20x)`
...
The least-squares estimators provide unbiased estimates of the $\beta$ parameters [@mendhall].
...
Here is an example of the `Bootstrap` function using the camplake data found in `library(s20x)`

```{r}
data(camplake.df)
Bootstrap(alpha=.95,iter=10000,model=Length~Age+Scale.Radius,data=camplake.df)
```
...
Here is an example of the `Bayesian` function using the camplake data found in `library(s20x)`

```{r}
library(s20x)
data(camplake.df)
Bayesian(model=Length~Age+Scale.Radius,data=camplake.df,alpha=.95)
```
...
Here is an example of the `ShinyMLR` function using the camplake data found in `library(s20x)`

```{r}
ShinyMLR(camplake.df)
```
